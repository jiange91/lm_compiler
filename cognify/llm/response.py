...
def aggregate_usages(usages: List[Usage]) -> CompletionUsage:
  aggregated_usage = CompletionUsage(prompt_tokens=0, 
                                     completion_tokens=0, 
                                     total_tokens=0, 
                                     prompt_tokens_details=PromptTokensDetails(audio_tokens=0, 
                                                                               cached_tokens=0),
                                     completion_tokens_details=CompletionTokensDetails(audio_tokens=0, 
                                                                                       reasoning_tokens=0))
  for usage in usages:
    ...
    if prompt_token_details:
      aggregated_usage.prompt_tokens_details += prompt_token_details.get('audio_tokens', 0)  
      aggregated_usage.prompt_tokens_details += prompt_token_details.get('cached_tokens', 0)

    completion_token_details = usage.get('completion_tokens_details', None)
    if completion_token_details:
      aggregated_usage.completion_tokens_details += completion_token_details.get('audio_tokens', 0)  
      aggregated_usage.completion_tokens_details += completion_token_details.get('reasoning_tokens', 0)
