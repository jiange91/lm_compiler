...
class AgentPropose(BaseModel):
    ...
    output: str = Field(
        description="content generated by the agent"
    )
...
def high_level_decompose_kernel(task: str, complexity: str):
    decompose_prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("human", "Original Single Agent Prompt:\n{prompt}\n\n")
        ]
    )
    llm = ChatOpenAI(model="gpt-4o", temperature=0.0)
    routine = decompose_prompt | llm
    new_agents_raw = routine.invoke({"prompt": task}).content
    
    # format to pydantic object
    format_instruction = get_pydantic_format_instruction(HighLevelDecompose)
    decompose_prompt.extend(
        [
            ("ai", "{new_agents}"),
            ("human", "Now please reformat your answer to the JSON format.\n{format_instructions}"),
        ]
    )
    parser = JsonOutputParser(pydantic_object=HighLevelDecompose)
    routine = decompose_prompt | llm | parser
    new_agents = routine.invoke({
        "prompt": task,
        "new_agents": new_agents_raw, 
...
def decompose_refine_kernel(new_agent_meta: dict[str, dict], semantic: LMSemantic):
    decompose_refine_prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("human", user_prompt),
              ...
              example_new_agents=high_level_new_agents_example,
    ...
    new_interaction = routine.invoke({
        ...
        "new_agents": new_agent_meta,
        }
    )
    decompose_refine_prompt.extend(
        [
            ("ai", "{new_interaction}"),
            ("human", "Now please reformat your answer in the desired format.\n{format_instructions}"),
        ]
    )
        ...
        if os.path.exists(log_path):
            ...
            def _ld(lm: LLMPredictor):
                new_agents_meta = {
                    ...
                    for name, meta in self.lm_2_new_agents[lm.name].agents.items()
                }
                new_system = decompose_refine_kernel(new_agents_meta, lm.semantic)
