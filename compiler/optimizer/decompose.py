...
from compiler.IR.program import Workflow, Input, Output, hint_possible_destinations
from compiler.IR.rewriter.utils import add_argument_to_position
...
class LMTaskDecompose:
    ...
    def _load_existing_decomposition(self, log_path, system: BaseModel, target_modules) -> dict:
        with open(log_path) as f:
            json_lm_to_system = json.load(f)
            lm_to_system = {k: system.model_validate(v) for k, v in json_lm_to_system.items()}
            self.decompose_target_lms = [
                m for m in self.lm_modules 
                if m.name in lm_to_system and (target_modules is None or m.name in target_modules)
            ]
            return lm_to_system

    def _get_decomposition_candidates(self, target_modules, log_dir: str) -> List[DecomposeCandidate]:
        logger.info("Estimating complexity of agents")

        agent_prompts = [m.get_agent_role() for m in self.lm_modules if target_modules is None or m.name in target_modules]
        complexity_list: List[ComplexityEstimation] = estimate_complexity_kernel(agent_prompts)
        
        decompose_candidates: List[DecomposeCandidate] = [DecomposeCandidate(lm, complexity.score, complexity.rationale) for lm, complexity in zip(self.lm_modules, complexity_list)]
        ...
        for candidate in decompose_candidates:
            ...
            logger.info(f"Complexity of {candidate.lm.name}: {candidate.score}\nrationale: {candidate.rationale}\n\n")
    ...
    def _high_level_decomposition(self, decompose_candidates: List[DecomposeCandidate], threshold: float) -> Dict[str, HighLevelDecompose]:
    ...
    def _mid_level_decomposition(self, high_level_result: Dict[str, HighLevelDecompose], log_path: str) -> Dict[str, NewAgentSystem]:
        ...
        def _ld(lm: CogLM):
            ...
            new_system: NewAgentSystem = decompose_refine_kernel(agent_metadata, lm.get_high_level_info())
    ...
    def _final_decomposition(self, log_path: str, mid_level_agent_system: Dict[str, NewAgentSystem]):
        ...
        for lm in self.decompose_target_lms:
            _fd(lm)
        with open(log_path, 'w+') as f:
            json_lm_to_final_system = {k: json.loads(v.model_dump_json()) for k, v in final_agent_system.items()}
    ...
    def prepare_decompose_metadata(
        ...
        if os.path.exists(log_path):
            ...
            new_agent_system = self._load_existing_decomposition(log_path, NewAgentSystem, target_modules)
        else:
            decompose_candidates: List[DecomposeCandidate] = self._get_decomposition_candidates(target_modules, log_dir)
            high_level_result: Dict[str, HighLevelDecompose] = self._high_level_decomposition(decompose_candidates, threshold)
            logger.info('High-level decomposition results:\n')
            pprint(high_level_result)
            
            new_agent_system = self._mid_level_decomposition(high_level_result, log_path)
        ...
        if os.path.exists(log_path):
            ...
            self.lm_to_final_system = self._load_existing_decomposition(log_path, StructuredAgentSystem, target_modules)
        else:
    ...
    def materialize_decomposition(
        ...
        for agent_name, agent_meta in new_agents.agents.items():
            ...
            if isinstance(agent_meta.output_json_schema, str):
                ...
                if 'END' not in agent_meta.next_action and len(output_model.model_fields) == 1:
            ...
            cog_agent.lm_config = default_lm_config if default_lm_config else lm.lm_config
            name_to_new_lm[agent_name] = cog_agent
            sub_graph.add_module(cog_agent)
        
        # Get static dependency edges
        agent_to_srcs = defaultdict(list, {agent_name: [input_name] for agent_name in new_agents.agents}) # default to input node
        for agent_name, agent_meta in new_agents.agents.items():
            ...
            if is_static_edge:
                for next_agent in next_action:
                    if next_agent == 'END':
                        ...
                        agent_to_srcs[name_to_new_lm[next_agent].name].append(agent_name) # for check name existance
