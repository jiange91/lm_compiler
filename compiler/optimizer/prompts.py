...
Also you should decide the output schema for each agent in the new system. To make you life easier, for any agent who only have a single string output, you can directly use the output variable name as the output schema. This relexation does not apply to agents whose next action can be 'END', as the final output should be structured to align with the original output schema. Unless the orignal output schema is also a variable name then you can use the same variable name as the schema.
...
Although these agents only output one string value, you should still generate valid json schema for each agent because their next action can be `End` so need to align with the orignal final output schema. Their output variables should be included as top-level properties in the schema. You are free to use any descriptive title and description for the schema.
