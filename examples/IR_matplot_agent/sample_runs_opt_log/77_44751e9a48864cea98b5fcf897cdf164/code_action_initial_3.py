...
def draw_ellipse(ax, mean, cov, color):
    ...
    ell = Ellipse(mean, v[0], v[1], angle=angle, color=color, alpha=0.2)  # Fixed the argument order
    ax.add_patch(ell)
...
for i in range(3):
    ...
    draw_ellipse(plt.gca(), scaler.inverse_transform(mean.reshape(1, -1)), cov, colors[i])  # Reshape mean for inverse transform
