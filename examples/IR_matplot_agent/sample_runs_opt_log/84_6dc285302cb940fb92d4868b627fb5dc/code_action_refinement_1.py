...
# Load data from CSV
data = pd.read_csv('data.csv', header=0)  # Read the CSV with headers
temp = data.iloc[:, 0].values  # Assuming column 1 is temperature
pressure = data.iloc[:, 1].values  # Assuming column 2 is pressure
solid_liquid_temp = data.iloc[:, 2].values  # Assuming column 3 is temperature for solid-liquid boundary
solid_liquid_pressure = data.iloc[:, 3].values  # Assuming column 4 is pressure for solid-liquid boundary
