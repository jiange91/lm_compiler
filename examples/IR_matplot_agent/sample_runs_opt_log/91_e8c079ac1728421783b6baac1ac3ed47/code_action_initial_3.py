...
for i, freq in enumerate(frequencies):
    ...
    # Create a filled area below the line
    ax.fill_between(time, 0, amplitude, where=(amplitude >= 0), alpha=0.1, color=colors[i])  # Fill below the line
    ax.fill_between(time, 0, amplitude, where=(amplitude < 0), alpha=0.1, color=colors[i])  # Fill below the line for negative values
