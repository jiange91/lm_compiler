...
You are now a code master. Please complete the following additional code, ensure the function handles inputs and matches the expected outputs as described (just continue writing and give your completed code)
'''
...
The nodes that can be transited to are: ['Receive_Incomplete_Code', 'Finalize_Code']. Please decide the next node that should be transited to. 
'''
...
code_completion_transit_semantic = LangChainSemantic(
  ...
  ['completed_code'],
  "output_node",
  output_format_instructions="Give your output in format <node>node_name</node> where node_name is the name of the next node chosen from ['Receive_Incomplete_Code', 'Finalize_Code']"
)

code_completion_transit_lm = LangChainLM('completion transit node', code_completion_transit_semantic, opt_register=True)
