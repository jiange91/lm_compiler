...
TRANSIT_NODE_PROMPT = '''
The nodes that can be transited to are: ['Finalize_Code', 'end_node']. Please decide the next node that should be transited to, and finally, output <node>node_name</node> where node_name is the name of the next node. Let's think step by step. If the current node's task of finalizing code is complete, then please consider transit to the end node. Otherwise, the task is not complete, stay at the current node.
'''
...
code_finalize_transit_lm = LangChainLM('transit node', code_finalize_transit_semantic, opt_register=True)
