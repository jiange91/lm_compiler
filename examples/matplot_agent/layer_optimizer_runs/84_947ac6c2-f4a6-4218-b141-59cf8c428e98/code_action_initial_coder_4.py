...
# Load the data
data = pd.read_csv("data.csv", header=None)  # Added header=None to handle no header in CSV
...
temperature_kelvin = phase_boundary[0].astype(float) + 273.15
pressure_bars = phase_boundary[1].astype(float) / 1e5

# Plot the phase boundaries
plt.plot(temperature_kelvin, pressure_bars, label='Solid-Liquid-Gas Boundary', color='blue')
plt.plot(solid_liquid_boundary[2].astype(float) + 273.15, solid_liquid_boundary[3].astype(float) / 1e5, label='Solid-Liquid Boundary', color='green')
