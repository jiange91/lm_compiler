...
# Check if the data has at least 4 columns
if data.shape[1] < 4:
    raise ValueError("The data does not have enough columns. Ensure the CSV file has at least 4 columns.")
...
temperature_kelvin = phase_boundary.iloc[:, 0] + 273.15
pressure_bars = phase_boundary.iloc[:, 1] / 1e5

# Plot the phase boundaries
plt.plot(temperature_kelvin, pressure_bars, label='Solid-Liquid-Gas Boundary', color='blue')
plt.plot(solid_liquid_boundary.iloc[:, 0] + 273.15, solid_liquid_boundary.iloc[:, 1] / 1e5, label='Solid-Liquid Boundary', color='green')

# Mark special points
plt.plot(273.16, 611.657 / 1e5, 'ro', label='Triple Point (273.16 K, 611.657 Pa)')
plt.plot(647.396, 22.064, 'ro', label='Critical Point (647.396 K, 22.064 MPa)')
