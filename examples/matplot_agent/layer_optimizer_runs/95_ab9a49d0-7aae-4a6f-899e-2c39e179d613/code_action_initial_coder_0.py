...
monthly_max_temps = filtered_data.groupby([filtered_data['Date'].dt.year, filtered_data['Date'].dt.month]).max().reset_index()
...
for year in monthly_max_temps['Date'].dt.year.unique():
    year_data = monthly_max_temps[monthly_max_temps['Date'].dt.year == year]
    temps = year_data['Temperature'].values
    months = year_data['Date'].dt.month.values - 1  # Convert to 0-based index
...
data_2015 = monthly_max_temps[monthly_max_temps['Date'].dt.year == 2015]
temps_2015 = data_2015['Temperature'].values
months_2015 = data_2015['Date'].dt.month.values - 1  # Convert to 0-based index
