...
class StormArticle(Article):
    ...
    def generate_article_with_ref(self) -> str:
        main_body = self.to_string()
        refs_2_idx = self.reference["url_to_unified_index"]
        sorted_refs = sorted(refs_2_idx.items(), key=lambda x: x[1])
        refs_list = [f"[{idx}] {url}" for url, idx in sorted_refs]
        refs_str = "# References\n\n" + "\n".join(refs_list)
        return main_body + "\n\n" + refs_str
    ...
    def dump_article_with_refs_as_plain_text(self, file_path):
        text = self.generate_article_with_ref()
        FileIOHelper.write_str(text, file_path)
