...
def cmp_rouge(gold_path, pred_path):
    for topic in os.listdir(pred_path):
        gold = open(os.path.join(gold_path, f'{topic}.txt'), 'r').read()
        pred = open(os.path.join(pred_path, topic, 'storm_polished_article.txt'), 'r').read()
        score = compute_rouge_scores(gold, pred)
        print(f'Topic: {topic}\n{score}')
    
cmp_rouge('storm_src/FreshWiki/txt', 'results')